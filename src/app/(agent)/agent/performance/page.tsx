...
import { useAgentMetrics, useAgentSettings, useUpdateAgentSettings } from "@/hooks/useAgentProfile";
...
export default function PerformancePage() {
  ...
  const { data: metricsData, isLoading: metricsLoading } = useAgentMetrics();
  ...
  const metrics = React.useMemo(
    () => (metricsData ? adaptAgentMetrics(metricsData) : null),
    [metricsData]
  );
  ...
  const toggleNotif = (key: keyof NonNullable<typeof settings>["notifications"]) => {
    if (!settings) return;
    void patchSettings({
    ...
    <div className="px-[20px] min-[980px]:px-[32px] py-[28px]">
      <div className="flex items-start justify-between gap-[16px] mb-[28px] flex-wrap">
        <div>
          ...
          <p className="font-mono text-[11px] tracking-[0.1em] uppercase text-[var(--ink-muted)]">
            THIS MONTH · {periodLabel.toUpperCase()} · YOUR OWN METRICS ONLY
          </p>
        </div>
      ...
      <section aria-label="Performance metrics" className="mb-[32px]">
        {metricsLoading ? (
          <div className="grid grid-cols-2 min-[760px]:grid-cols-4 gap-[12px]">
            {[1, 2, 3, 4].map((i) => (
              <SkeletonBlock key={i} className="h-[88px] rounded-[var(--r-lg)]" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 min-[760px]:grid-cols-4 gap-[12px]">
            ...
            <StatTile
              ...
              value={metrics ? `${metrics.avgTurnaround}d` : "—"}
              delta={metrics?.avgTurnaroundDelta ?? "—"}
            />
            <StatTile
              label="On-time SLA"
              value={metrics ? `${metrics.onTimeSLAPercent}%` : "—"}
              delta="target 90%"
              deltaColor="muted"
            />
            <StatTile
              label="Client rating"
              value={metrics?.ratingsCount ? metrics.clientRating.toFixed(1) : "—"}
              delta={
                metrics?.ratingsCount
                  ? `${metrics.ratingsCount} ratings`
                  : "no ratings yet"
              }
              variant="brand"
            />
      ...
      <section aria-label="Performance charts" className="mb-[40px]">
        <div className="grid min-[900px]:grid-cols-[1.4fr_1fr] gap-[20px]">
          <div className="bg-[var(--paper)] rounded-[var(--r-lg)] border border-[var(--rule)] p-[20px]">
            ...
            <PerfBarChart bars={metrics?.weeklyBars ?? []} />
          </div>
          ...
          <div className="bg-[var(--paper)] rounded-[var(--r-lg)] border border-[var(--rule)] p-[20px]">
            ...
            <Leaderboard entries={metrics?.leaderboard ?? []} />
          </div>
        </div>
      </section>
      ...
      <section id="settings" aria-label="Agent settings" className="max-w-[680px]">
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-[8px] mt-8",
