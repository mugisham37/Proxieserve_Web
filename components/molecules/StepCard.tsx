...
export function StepCard({ item }: StepCardProps) {
  return (
    <div className="reveal bg-surface border border-border rounded-[14px] p-6 relative">
      <div className="font-mono text-[12px] text-text-subtle mb-3">{item.num}</div>
      <div className="w-10 h-10 rounded-[10px] bg-brand-soft text-brand flex items-center justify-center mb-3">
        <MkIcon name={item.icon} size={20} />
      </div>
